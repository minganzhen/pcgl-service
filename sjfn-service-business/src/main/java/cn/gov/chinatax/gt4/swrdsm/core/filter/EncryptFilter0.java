...
@AllArgsConstructor
@Slf4j
public class EncryptFilter0 extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        try {
            ...
            if (HttpMethod.GET.name().equalsIgnoreCase(request.getMethod())) {
                ...
                Map<String, String> map = this.buildParam(encryptParam);
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        if (uri.startsWith(apiPrefix)) {
            return !properties.getInterceptUrls().stream().anyMatch(excludeUrl -> pathMatcher.match(excludeUrl, uri));
        }
        return true;
    }
    ...
    private void checkParam(ServletResponse response, EncodeResponseWapper responseWapper) throws IOException {
        ...
        objectObjectHashMap.put("0", responseData);
        writer.print(JsonUtil.toJsonString(objectObjectHashMap));
        writer.flush();
        writer.close();
    }
